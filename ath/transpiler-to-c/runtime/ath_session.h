...
/* ath_session.h -- loaded shared library + transcribed rites.
   Sessions are the !~ATH FFI mechanism: a session value carries a dlopen handle, a map of transcribed rites (callable as M.foo(...)), and a registry of relics minted by those rites. Sessions are paired with an AthEntity so they participate in death waiting via ~ATH(M). */
...
/* Relic registry. Relics call register on construction and unregister on their own destruction. The session does not own the relics; it just keeps weak pointers so it can curse them on death. */
...
/* Queue the session's teardown continuation. ath_entity_die calls this after marking the session dying; the continuation runs at the next event-loop tick. Orderly teardown runs DROPS destructors in reverse creation order then dlcloses; fault teardown (session->faulted set by the signal handler) curses every relic, skips destructors, and leaks the mapping. Either way, the session's entity is marked dead and ~ATH(M) waiters fire only after teardown completes. */
